...
- **Rails:** `AppSidebar` (dashboard / admin, `expanded` or `rail`), `ProjectSidebar` (project / document tools: workspace switcher, ⌘K search, groups with add, row status dots and menus, `layout="edge" | "inset"` + `ProjectInset`) or `SidebarNav` (chat), inside `<div className="flex h-dvh">…<main className="min-w-0 flex-1 overflow-y-auto">`. Never build a sidebar from divs.
...
- **App configuration:** `src/formic/formic.config.json` (accent, palette, radius, size, starting theme, avatar kind initials/doodle/photo, sidebar variant, motion) is the source of truth. When the user pastes a block from formicai.dev/customize, save the JSON there verbatim and run `python3 src/formic/scripts/apply_config.py`; it fits the accent, sets the `<html data-*>` attributes and writes `components/config.ts` (the prop defaults). "Rounder corners", "start dark", "use doodle avatars" are config edits, not inline props. Never override a config value inline.
