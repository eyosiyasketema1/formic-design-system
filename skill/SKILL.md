...
- **Global scales:** `data-radius` (sharp / rounded / full, or `custom` from a pixel value in the config), `data-corners` (round instead of the default squircle) and `data-size` (comfortable / spacious) are token overrides like palettes. Components never read them.
