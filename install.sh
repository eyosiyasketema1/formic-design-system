...
const PROMPTS: { label: string; title: string; caption: string; text: string }[] = [
  ...
  },
  {
    ...
    caption: "A conversation with the agent's work beside it; the AI-product shape",
    text: `${PREFIX} replace the welcome page (App.tsx is yours to change; keep main.tsx and CustomizeNudge.tsx as they are) with an agent workspace for Formic Studio, composed the way AGENTS.md (App shell; An agent screen is a conversation, not a pile; Nothing moves on its own) says: an AppShell with \`rail="chat"\` (the chat rail, never the dashboard rail), the conversation's title in the strip (a follow-up for a client) with one action (Share); the thread in one \`max-w-2xl\` column with the demo conversation, where the assistant's last turn is, in order, a finished ThinkingState (collapsed, opens on click), TaskRows with one row still active, an ApprovalCard whose Approve and Reject both resolve it, then the answer as Markdown; the PromptBar pinned under the thread at the same width, sending appends the message and shows \`status="streaming"\` with a Stop button until a short reply streams in; beside the thread, in a SplitPane, one Panel titled after the file the agent is editing holding a FileTree and a CodeBlock. Nothing runs, opens or changes on its own: no \`demo\` props, no timers except the reply after a send. The theme switch above the account row flips, the pane divider drags, and the panes stack on a narrow screen. ${CLOSE}`,
  },
  {
    label: "Test prompt 3",
    title: "Client record",
