...
// email carries the address and an HMAC of it under WAITLIST_SECRET, so nobody
// can unsubscribe someone else by guessing. Unsubscribed addresses go into a set
...
module.exports = async function handler(req, res) {
  ...
  // the token is an HMAC of the address under WAITLIST_SECRET; no secret, no valid links
  const secret = process.env.WAITLIST_SECRET || "";
  const expected = secret ? crypto.createHmac("sha256", secret).update(email).digest("hex").slice(0, 32) : "";
  const valid = Boolean(secret) && token.length === expected.length && crypto.timingSafeEqual(Buffer.from(token), Buffer.from(expected));
  ...
  if (!email || !valid) return page("That link does not work", "It may be incomplete. Reply to any email from us and we will remove you by hand.");
