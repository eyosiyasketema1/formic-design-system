...
**Dashboard:** Panel (the titled card), StatCard (layouts label-first / value-first / chart-middle / inline, compact `size="sm"` tiles, trend as a line, capsule bars or a two-series stack, a progress `ring`, `align="center"` heroes, a `chart` slot), MetricRow (`leading` tile, `progress`, `big`, `trend`), Delta, Progress with `segments`, BarChart (`highlight`, values above each bar with `valuePosition="bar"`), LineChart (`curve="step"`, `backdrop`, `guides`), MiniBars, DonutChart, Sparkline, ChartLegend, CountUp, Gauge, BarList, PrivacyScope / PrivacyToggle / Masked. The gallery's Dashboard page composes a full finance overview from these; copy its shapes rather than inventing tiles.
