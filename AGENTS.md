...
**Navigation:** AppShell (the rail from the config plus the page header and `.page-content`; every page, `rail="none"` for one that stands alone), CommandPalette (`CommandPalette.tsx`: ⌘K for the whole app; `useCommandPalette()` wires the key and returns `open` / `show` / `close`; `groups` of `Command` {key, label, hint?, icon?, keys?, data?}, a `recent` group shows on an empty query; `onSelect(item, group)`; `search={(q) => groups}` for an index of your own; it is the search behind the rails' ⌘K), Tabs (underline for page sections, segmented for views of one thing, subtle for filters), Pagination, Breadcrumbs, Menubar, TopBar (the strip above a page: title, a leading trigger, search, theme, notifications, the account menu; pair with `<AppSidebar user={null} />` for the rail-plus-bar shell), AppSidebar (dashboard rail, expanded / rail, sub-menus), ProjectSidebar (workspace rail: switcher, search, groups, row menus, edge / inset), SidebarNav (chat rail), SearchList.

**App shell.** Every page is an `AppShell`, no exceptions; it mounts the rail the config chose and renders the page header, so the page passes `title`, `caption`, `actions` and writes no `<h1>` and no rail of its own. The config says what the app's rail looks like; the page's kind says whether it has one: pages inside the app take the rail from the config, a page that stands alone (sign-in, onboarding, a public form, a shared document) takes `rail="none"` and gets a slim header strip instead. Either way the theme switch is in the shell, in the same corner on every page (beside the profile, in the TopBar, or at the right of the strip), so the reader can always change light and dark and you never add a switch of your own:
...
`SidebarNav` (chat) is the one rail outside it: `<div className="flex h-dvh"><SidebarNav … /><main className="min-w-0 flex-1 overflow-y-auto">…</main></div>`. Rails fill their parent's height; never give one a fixed height or put it inside a scrolling page.
