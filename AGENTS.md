...
**Feedback and agents:** EmptyState (what a surface says when there is nothing to show: `kind` first / search / filter / error with its own icon and words, `query` quoted for search, one `action`, a quiet `secondary`, `size` sm inside a panel or table and md for a page; DataTable renders it by default), Alert, Progress, Skeleton, LoadingState, ThinkingIndicator (one line: breathing glyph + a rotating shimmer word, for the gap before a reply), ThinkingState (steps, reasoning, sources), TaskRows, ToolChips.
