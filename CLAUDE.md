...
11. **Control metrics.** Interactive controls share one height scale: 24 (xs) / 32 (sm) / 36 (md) / 40 (lg) — a md button lines up with a md input, always. Horizontal padding ≈ height/3 rounded to the spacing scale. Button-like labels use `leading-none` so the font's line box can't float the text off optical center. Fills are flat — no inner sheens or bevel highlights on interactive elements (`--highlight-raised` was retired for this).
